...
class Uzivatel extends Nette\Object
{
  ...
  /** @return string */
    ...
    /** @return Nette\Database\Table\Selection */
    public function zakoupeneSkladby($uziv, $typ)
    {
        $table = $typ == "flasinet" ? "flasinet_nakup" : "nakup";
        return $this->database->table($table)->where('uzivatel_id', $uziv);
    }

    /** @return integer */
    public function sumaNakupu($uziv)
    {
        $midi = $this->zakoupeneSkladby($uziv, "midi")->sum('cena');
        $flasinet = $this->zakoupeneSkladby($uziv, "flasinet")->sum('cena');
        $suma = $midi + $flasinet;
        return $suma ?: 0;
    }

    /** @return boolean */
    public function maZakoupeno($uziv, $skladba, $typ)
    {
        return $this->zakoupeneSkladby($uziv, $typ)->where('skladba_id', $skladba)->count() ? true : false;
    }
  ...
  /** @return Nette\Database\Table\ActiveRow */
    ...
    public function koupitSkladbu($uzivId, $skladba, $typ)
    {
      $tableNakup = $typ == "flasinet" ? "flasinet_nakup" : "nakup";
      $tableSkladba = $typ == "flasinet" ? "flasinet_skladba" : "skladba";
      ...
      $this->database->table($tableNakup)->insert($arr);

      $this->database->table('uzivatel')
        ->where('id', $uzivId)
        ->update(array('kredit' => $uziv->kredit - $skladba->cena));

      $this->database->table($tableSkladba)
        ->where('id', $skladba->id)
