...
class Flasinet extends Nette\Object
{
    ...
    {
        $skladby = $this->database->table('flasinet_skladba');
    if($filtry['nazev']) {
  ...
  /** @return array */
    ...
    {
    return $this->database->table('flasinet_zanr')->where('aktivni', 1)->fetchPairs('id', 'nazev');
    }

  /** @return array */
    public function seznamFormatu($demo = null)
    {
    return $this->database->table('flasinet_format')->where('demo', $demo == 'demo' ? 1 : 0)->where('aktivni', 1)->fetchPairs('id', 'nazev');
    }
    ...
    public function prehledStahovani($od, $do, $razeni = null, $limit = null, $offset = null)
    {
        ...
        $skladby = $this->database->table('flasinet_nakup')
        ->select('skladba_id, skladba.nazev AS nazev, skladba.autor AS autor, flasinet_nakup.cena AS cena, count(*) AS pocet')
        ->where('datum >= ?', $od)
        ->where('datum <= ?', $do)
        ->group('skladba_id');
    ...
    {
    return $this->database->table('flasinet_skladba')->order('pocet_stazeni DESC')->limit(10);
    }
    ...
    {
        $this->database->table('flasinet_skladba')->wherePrimary($skladbaId)->update($values);
    }
  ...
  /** @return Nette\Database\Table\ActiveRow */
    ...
    return $this->database->table('flasinet_skladba')->insert($skladba);
    }

  /** @return Nette\Database\Table\ActiveRow */
    public function delete($skladbaId)
    {
    return $this->database->table('flasinet_skladba')->wherePrimary($skladbaId)->delete();
    }
  ...
  /** @return Nette\Database\Table\ActiveRow */
    ...
    foreach ($soubory as $soubor) {
      $this->database->table('flasinet_soubor')->where('skladba_id', $soubor['skladba_id'])->where('format_id', $soubor['format_id'])->delete();
    }
    return $this->database->table('flasinet_soubor')->insert($soubory);
    }

    public function nazevSouboru($id)
    {
    return $this->database->table('flasinet_soubor')->get($id);
    }

    /** @return null */
    public function exportNazvuSkladeb($soubor)
    {
        $skladby = $this->database->table('flasinet_skladba')->select('id,nazev')->order('nazev');
        $eol = "\r\n";
    ...
    {
        $skladby = $this->database->table('flasinet_skladba')->select('DISTINCT autor')->order('autor');
        $eol = "\r\n";
        $handle = fopen('safe://' . $soubor, 'w');
        fwrite($handle, '[' . $eol);
        foreach ($skladby as $skladba) {
    ...
    {
        return $this->database->table('flasinet_nakup')->where("uzivatel.role", "zakaznik")->count();
    }

    public function celkemNakoupenoZa()
    {
        return $this->database->table('flasinet_nakup')->where("uzivatel.role", "zakaznik")->sum("cena");
    }
}
