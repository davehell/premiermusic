 ...
 * Flasinet presenter.
 */
class FlasinetPresenter extends BasePresenter
{
  ...
  public function skladbaFormSucceeded($form)
  {
    ...
    $this->skladby->exportNazvuSkladeb($this->context->parameters['wwwDir'] . '/flasinet-skladby.json');
    $this->skladby->exportAutoru($this->context->parameters['wwwDir'] . '/flasinet-autori.json');

    $this->redirect('Flasinet:detail', $skladbaId);
  }
  ...
  {
    $values = $form->getValues();
    $params = array('nazev' => $values['nazev'], 'autor' => $values['autor'], 'zanr' => $values['zanr']);
    if(!$params['nazev']) $params['nazev'] = null;
    if(!$params['autor']) $params['autor'] = null;
    $this->redirect('Flasinet:katalog', $params);
  }
  ...
  public function actionNakup($id)
    ...
    if($this->uzivatele->maZakoupeno($this->user->id, $id, "flasinet")){
      $this->redirect('Flasinet:detail', $id);
    }

    try {
      $this->uzivatele->koupitSkladbu($this->user->id, $skladba, "flasinet");
    } catch (\Exception $e) {
      $this->flashMessage($e->getMessage(), 'danger');
      $this->redirect('Flasinet:detail', $id);
    }

    BasePresenter::sendMail('flasinet-nakup.latte', $this->user->getIdentity()->data['email'], $skladba);
  }
  ...
  public function actionPridat()
  {
    ...
    if(!$soubor->format->demo) {
      ...
      if(!$this->uzivatele->maZakoupeno($this->user->id, $soubor->skladba_id, "flasinet")){
        $this->flashMessage('Tuto skladbu nemáte zakoupenou.', 'danger');
        $this->redirect('Flasinet:detail', $soubor->skladba_id);
      }
    }
