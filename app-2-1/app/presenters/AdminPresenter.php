...
class AdminPresenter extends BasePresenter
{
  ...
  public function formatLayoutTemplateFiles()
  {
      ...
      $layoutFiles[] = "$dir/../../presenters/templates/@midi.latte";
  ...
  public function hledaniFormSucceeded($form)
  {
    ...
    public function renderStahovani($zacatek = null, $konec = null)
    {
      ...
      $pocetSkladebFlasinety = $this->flasinety->prehledStahovani($this->dmyToYmd($zacatek), $this->dmyToYmd($konec))->count();
      $vp2 = new VisualPaginator($this, 'vp2');
      $paginator2 = $vp2->getPaginator();
      $paginator2->itemsPerPage = 50;
      $paginator2->itemCount = $pocetSkladebFlasinety;

      $this->template->nakupyMidi = $this->skladby->prehledStahovani($this->dmyToYmd($zacatek), $this->dmyToYmd($konec), $razeni, $paginator->getLength(), $paginator->getOffset());
      $this->template->nakupyFlasinet = $this->flasinety->prehledStahovani($this->dmyToYmd($zacatek), $this->dmyToYmd($konec), $razeni, $paginator->getLength(), $paginator->getOffset());
    ...
    public function actionStahovaniDownload($zacatek, $konec, $typ)
    {
      if($typ == "flasinet") {
        $nakupy = $this->flasinety->prehledStahovani($this->dmyToYmd($zacatek), $this->dmyToYmd($konec), array('sloupec' => 'nazev', 'smer' => 'ASC'));
      }
      else {
